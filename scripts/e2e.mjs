...
const englishScan = `(() => { const found = []; const walk = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT); let node; while ((node = walk.nextNode())) { const el = node.parentElement; if (!el || el.closest('script,style,svg,[aria-hidden=true]')) continue; const text = node.textContent.trim(); if (text && /[A-Za-z]{3,}/.test(text) && !/[А-Яа-яЁё]/.test(text) && !/^(PathTry|PathFinder|pathtry|path|try|O\\*NET.*|ESCO|AI|EN|RU|IELTS.*|SAT.*|IB.*|A-Levels.*|MCAT.*|LSAT.*|GRE.*|NCLEX.*|UX.*|CompTIA.*|Enter|Ctrl.*|Model United Nations|Godot|Unity|CTF|PATHFINDER.*|GitHub|Tengrinews.*|WEF.*|European ESCO|OECD Career Readiness|Harvard GSE.*|OECD — Career Readiness project|Harvard Graduate School of Education.*|Tair Idrissov|Damir Omar|Amelya Leonova|Arlan Sharipov)$/.test(text)) found.push(text); } return found; })()`;
