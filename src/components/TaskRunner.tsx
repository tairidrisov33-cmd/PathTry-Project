...
export default function TaskRunner({ definition }: { definition: ProfessionDef }) {
  ...
  return <div>
    ...
    <div className="progress-steps" style={{ '--n': total } as React.CSSProperties} role="progressbar" aria-label={ru ? 'Прогресс заданий' : 'Task progress'} aria-valuemin={0} aria-valuemax={total} aria-valuenow={step} aria-valuetext={`${step} / ${total}`}>{profession.tasks.map((item, index) => <span key={item.id} className={index < step ? progressClass(answers[index]) : index === step ? 'current' : ''} />)}</div>
