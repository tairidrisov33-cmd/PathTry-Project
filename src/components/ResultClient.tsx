...
export default function ResultClient({ definition, alternative }: { definition: ProfessionDef; alternative: ProfessionDef }) {
  ...
  return <>
    ...
    <section className="dashboard-grid">
      ...
      <div className="dashboard-panel ai-breakdown"><div className="ai-orb"><Icon name="compassNav" size={22} /></div><div className="panel-kicker">{ru ? 'PATHFINDER · AI-РАЗБОР' : 'PATHFINDER · AI CAREER BREAKDOWN'}</div><h2>{ru ? 'Разбор твоего эксперимента' : 'Your experiment, decoded'}</h2><div className="ai-thinking"><span /> <span /> <span /> <em>{ru ? 'Собираем сигналы из ответов' : 'Reading signals from your answers'}</em></div><div className="ai-stat"><span><Icon name={strongest.icon} size={14} />{ru ? 'Сильные стороны' : 'Strengths'}</span><strong>{`${ranked[0].label}, ${ranked[1].label.toLowerCase()}.`}</strong></div><div className="ai-match"><strong><CountUp value={match} suffix="%" /></strong><span>{ru ? 'совпадение с рабочим стилем' : 'work style match'}</span></div><div className="match-bar"><span style={{ width: `${match}%` }} /></div><p className="ai-alternative"><Icon name="route" size={14} />{ru ? 'Похожее направление, которое стоит попробовать: ' : 'A related path worth trying: '}<TransitionLink href={`/try/${alternative.slug}`}>{alternative.title[lang]} <Icon name="arrowUpRight" size={13} /></TransitionLink></p></div>
    </section>
    ...
    {showCard && <VerificationCard slug={definition.slug} profession={profession.title} match={match} strongest={strongest.label} skills={skills} language={language} onClose={() => setShowCard(false)} />}
  </>;
}
