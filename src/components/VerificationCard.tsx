...
export default function VerificationCard({ profession, match, strongest, skills, language, onClose }: { profession: string; match: number; strongest: string; skills: SkillPoint[]; language: 'en' | 'ru'; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label={ru ? 'Карточка подтверждения' : 'Verification card'}><div className="verification-modal"><button className="modal-close" type="button" onClick={onClose} aria-label={ru ? 'Закрыть' : 'Close'}>×</button><div className="verification-card" ref={cardRef}><div className="verification-brand">✦ PATHTRY</div><div className="verification-name">{name || (ru ? 'Твой профиль' : 'Your profile')}</div><div className="verification-profession">{profession}</div><div className="verification-divider" /><div className="verification-stat"><strong>{match}%</strong><span>{ru ? 'совпадение рабочего стиля' : 'work style match'}</span></div><div className="verification-skill">{ru ? 'Сильная сторона' : 'Strength'}: <b>{strongest}</b></div><div className="verification-qr"><img src={`https://api.qrserver.com/v1/create-qr-code/?size=90x90&data=${encodeURIComponent(shareUrl)}`} alt="QR code" /></div></div><label className="verification-name-input">{ru ? 'Имя на карточке' : 'Name on card'}<input value={name} onChange={(event) => setName(event.target.value)} placeholder={ru ? 'Например, Анна' : 'For example, Alex'} /></label><div className="verification-actions"><button className="btn btn-primary" type="button" onClick={copyImage}>{ru ? 'Скопировать карточку' : 'Copy Image / Link'}</button><button className="btn btn-outline" type="button" onClick={copyLink}>{ru ? 'Скопировать ссылку' : 'Copy Link'}</button><button className="btn btn-outline" type="button" onClick={share}>{ru ? 'Поделиться' : 'Share to Socials'}</button></div>{status && <p className="modal-status">{status}</p>}</div></div>;
}
